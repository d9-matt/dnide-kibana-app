  ...
  if (currentApi && currentApi.id) {
    ...
    if (!apiVersion) {
      ...
      if (
        ...
        checkLogger.error(`Wazuh API and Wazuh App version mismatch. API version: ${apiVersion}. App version: ${setupData.data.data['app-version']}. At least, major and minor should match. Check more info about upgrading Wazuh App <a target='_blank' href='https://documentation.portal9.com/current/upgrade-guide/elasticsearch-kibana-filebeat/index.html#upgrade-elasticsearch-filebeat-kibana'>here</a>.`);
      }
    }
  }
