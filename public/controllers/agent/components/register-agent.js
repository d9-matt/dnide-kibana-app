...
export class RegisterAgent extends Component {
  ...
  obfuscatePassword(text) {
    let obfuscate = '';
    const regex = /PORTAL9_REGISTRATION_PASSWORD=?\040?\'(.*?)\'/gm;
  ...
  optionalDeploymentVariables() {
    let deployment = `PORTAL9_MANAGER='${this.state.serverAddress}' `;
    if (this.state.selectedOS == 'win') {
      deployment += `PORTAL9_REGISTRATION_SERVER='${this.state.serverAddress}' `;
    }

    if (this.state.needsPassword) {
      deployment += `PORTAL9_REGISTRATION_PASSWORD='${this.state.portal9Password}' `;
    }

    if (this.state.udpProtocol) {
      deployment += `PORTAL9_PROTOCOL='UDP' `;
    }

    if (this.state.selectedGroup.length) {
      deployment += `PORTAL9_AGENT_GROUP='${this.state.selectedGroup.map((item) => item.label).join(',')}' `;
    }
