...
import { PORTAL9_MONITORING_PATTERN, PORTAL9_SAMPLE_ALERT_PREFIX, PORTAL9_ROLE_ADMINISTRATOR_ID, PORTAL9_SAMPLE_ALERTS_INDEX_SHARDS, PORTAL9_SAMPLE_ALERTS_INDEX_REPLICAS } from '../../common/constants';
...
import { PORTAL9_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS, PORTAL9_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS } from '../../common/constants'
...
export class Portal9ElasticCtrl {
  ...
  getSampleAlertPrefix(): string {
    ...
    return config['alerts.sample.prefix'] || PORTAL9_SAMPLE_ALERT_PREFIX;
  }
  ...
  async buildVisualizationsRaw(app_objects, id, namespace = false) {
    try {
      ...
      let monitoringPattern =
        (config || {})['portal9.monitoring.pattern'] || PORTAL9_MONITORING_PATTERN;
      log(
  ...
  async haveSampleAlerts(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      ...
      const results = await Promise.all(Object.keys(PORTAL9_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS)
  ...
  async createSampleAlerts(context: RequestHandlerContext, request: KibanaRequest<{ category: string }>, response: KibanaResponseFactory) {
    ...
    try {
      ...
      const sampleAlerts = PORTAL9_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS[request.params.category].map((typeAlert) => generateAlerts({ ...typeAlert, ...alertGenerateParams }, request.body.alerts || typeAlert.alerts || PORTAL9_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS)).flat();
      ...
      if (!existsSampleIndex.body) {
        ...
        const configuration = {
          settings: {
            index: {
              number_of_shards: PORTAL9_SAMPLE_ALERTS_INDEX_SHARDS,
              number_of_replicas: PORTAL9_SAMPLE_ALERTS_INDEX_REPLICAS
            }
          }
        };
